...
# # --- 1. Prompt Parsing ---
# def parse_prompt(prompt, api_key, carpet_area=None):
#     client = OpenAI(api_key=api_key)

#     system_instruction = (
#         "You are an expert architect AI assistant. "
#         "Return ONLY a valid JSON list of room dictionaries with 'room_type', 'width', and 'height'. "
#         "No explanation, no text outside JSON. "
#     )

#     if carpet_area:
#         system_instruction += f"The total carpet area of the house is {carpet_area} square meters."
...
#         print("🔎 GPT response raw content:", repr(content))
...
#         print("❌ Prompt parsing failed:", e)
#         return []

# # --- 2. Graph Preparation for GATNet ---
# def prepare_graph(room_data):
#     node_features = []
#     edges = []

#     for i, room in enumerate(room_data):
#         room_type = hash(room["room_type"]) % 1000 / 1000.0
#         width = room.get("width") or 4.0
#         height = room.get("height") or 4.0
#         node_features.append([room_type, width, height] + [0]*6)
...
# # --- 3. Dummy Boundary Graph ---
# def prepare_dummy_boundary(num_nodes):
...
# # --- 4. Layout Drawing with Doors + Boundary ---
# def draw_layout(room_data, pred_widths, pred_heights):
#     room_colors = {
#         'bedroom': '#ADD8E6',
#         'kitchen': '#FFD580',
#         'bathroom': '#FFB6C1',
...
#     for i, room in enumerate(room_data):
#         width = round(room.get("width") or pred_widths[i], 1)
#         height = round(room.get("height") or pred_heights[i], 1)
...
#         color = room_colors.get(room_type, '#d3e5ff')
...
#         # Label
#         ax.text(x + width / 2, y + height / 2, room["room_type"].title(),
#                 ha='center', va='center', fontsize=10, color='black', weight='bold')

#         # Dimension annotations
#         ax.text(x + width / 2, y + height + 0.3, f"{width}m", ha='center', fontsize=8)
#         ax.text(x + width + 0.3, y + height / 2, f"{height}m", va='center', fontsize=8, rotation=90)

#         # Door
#         ax.add_patch(patches.Rectangle((x + width/2 - 0.5, y - 0.2), 1.0, 0.2, color="brown"))

#         max_width = max(max_width, x + width)
#         max_y = min(max_y, y - height)

#     # Draw outer boundary box
#     boundary = patches.Rectangle((-1, max_y - 1), max_width + 3, -max_y + 5,
#                                  linewidth=3, edgecolor='black', facecolor='none', linestyle='--')
#     ax.add_patch(boundary)

#     # Layout formatting
#     ax.set_xlim(-2, max_width + 4)
#     ax.set_ylim(max_y - 4, 5)
#     ax.set_aspect('equal')
#     ax.axis('off')
...
#     layout_path = os.path.join(layout_dir, f"layout_{uuid.uuid4().hex[:6]}.png")
