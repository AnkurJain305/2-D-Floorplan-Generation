# --- Torch class patch for Streamlit ---
import sys
import types
import os
...
def main():
    ...
    with st.expander("A Deep Dive into Our Graph-Based Floor Plan Generator"):
        st.write("""
            ## Graduation Project: Residential Floor Plan Generation Using Deep Learning Techniques
    ...
    # Evaluate the string inputs into actual Python lists of tuples
        try:
            room_centroids_eval = eval(room_input)
            bath_centroids_eval = eval(bath_input)
            kitchen_centroids_eval = eval(kitchen_input)
        
            output_path, Graph_netorkX, Boundary_networkX = Run(
                boundary_wkt,
                front_door_wkt,
                room_centroids_eval,
                bath_centroids_eval,
                kitchen_centroids_eval
            )

            st.image(output_path, caption='Generated Floor Plan Output', use_column_width=True)
...
# import streamlit as st
# from main import generate_layout_from_prompt
# from PIL import Image
...
# # --- Page Setup ---
# st.set_page_config(page_title="Room Layout Generator", layout="centered")
# st.title("🏠 Room Layout Generator using GATNet + GPT")

# # --- Input Form ---
# with st.form("floor_form"):
#     prompt = st.text_area("🧠 Describe your room requirements (e.g., 2 bedrooms, 1 kitchen, 1 bathroom)", height=150)
...
#     rooms = [
#         {"name": "Bedroom", "x": 0, "y": 4, "width": 4, "height": 4, "color": "lightblue"},
#         {"name": "Toilet", "x": 4, "y": 5, "width": 1, "height": 2, "color": "lightgreen"},
#         {"name": "Living Room", "x": 5, "y": 4, "width": 5, "height": 4, "color": "lightsteelblue"},
#         {"name": "Bedroom", "x": 0, "y": 0, "width": 4, "height": 4, "color": "lightblue"},
#         {"name": "Bathroom", "x": 4, "y": 2, "width": 2, "height": 2, "color": "pink"},
#         {"name": "Kitchen", "x": 6, "y": 0, "width": 4, "height": 4, "color": "peachpuff"},
#     ]
...
#                          facecolor=room["color"], edgecolor='black', linewidth=2)
#         ax.add_patch(rect)

#         ax.text(room["x"] + room["width"]/2, room["y"] + room["height"]/2,
#                 room["name"], ha='center', va='center', fontsize=10, weight='bold')

#         ax.text(room["x"] + room["width"]/2, room["y"] + room["height"] + 0.2,
#                 f"{room['width']}m", ha='center', va='bottom', fontsize=8)

#         ax.text(room["x"] - 0.2, room["y"] + room["height"]/2,
#                 f"{room['height']}m", ha='right', va='center', fontsize=8, rotation=90)
...
#     outer_rect = Rectangle((min_x - 0.5, min_y - 0.5), (max_x - min_x + 1), (max_y - min_y + 1),
#                            edgecolor='black', facecolor='none', linewidth=3, linestyle='--')
#     ax.add_patch(outer_rect)

#     ax.set_title("Sample Floor Plan Layout", fontsize=14, weight='bold')

#     legend_elements = [
#         Patch(facecolor='lightblue', edgecolor='black', label='Bedroom'),
#         Patch(facecolor='pink', edgecolor='black', label='Bathroom'),
#         Patch(facecolor='peachpuff', edgecolor='black', label='Kitchen'),
#         Patch(facecolor='lightgreen', edgecolor='black', label='Toilet'),
#         Patch(facecolor='lightsteelblue', edgecolor='black', label='Living Room'),
#     ]
...
#     st.image(buf, caption="Fallback Layout", use_column_width=True)
#     st.download_button("📥 Download Sample Layout", data=buf, file_name="sample_floor_plan.png", mime="image/png")
#     plt.close()
...
#     else:
#         try:
#             with st.spinner("🤖 Generating layout using GPT + GATNet..."):
#                 layout_path = generate_layout_from_prompt(prompt, api_key)

#             if layout_path and os.path.exists(layout_path):
#                 st.success("✅ Layout generated successfully!")
#                 st.image(layout_path, caption="Generated Floor Plan", use_column_width=True)
#                 with open(layout_path, "rb") as f:
#                     st.download_button("📥 Download Floor Plan", data=f, file_name="floor_plan.png", mime="image/png")
#             else:
#                 st.warning("⚠️ AI-based layout generation failed. Showing static layout instead.")
#                 draw_static_layout()
...
#     prompt = st.text_area("🧠 Describe your room requirements (e.g., 2 bedrooms, 1 kitchen, 1 bathroom)", height=150)
#     api_key = st.text_input("🔐 Enter your OpenAI API Key", type="password")
#     use_carpet = st.checkbox("📐 Add total carpet area")
#     carpet_area = None
#     if use_carpet:
#         carpet_area = st.number_input("Enter total carpet area in m²", min_value=20.0, value=60.0, step=5.0)
#     submitted = st.form_submit_button("✨ Generate Layout")

# # --- Fallback Layout ---

# def draw_static_layout_with_carpet(carpet_area):
#     import matplotlib.pyplot as plt
#     from matplotlib.patches import Rectangle, Patch
#     from io import BytesIO
#     import streamlit as st
...
#     st.subheader("📐 Optimized Floor Plan based on Carpet Area")
...
#     room_data = []
#     for name, ratio in room_ratios.items():
#         area = carpet_area * ratio
#         width = round(math.sqrt(area), 2)
#         height = round(area / width, 2)
#         room_data.append({"name": name, "width": width, "height": height, "color": colors[name]})
...
#         room["x"] = x
#         room["y"] = y
#         max_row_height = max(max_row_height, room["height"])

#         x += room["width"] + spacing
#         if (idx + 1) % cols == 0:
#             x = 0
...
#     total_width = max(room["x"] + room["width"] for room in room_data)
#     total_height = max(room["y"] + room["height"] for room in room_data)
#     ax.set_xlim(-1, total_width + 1)
#     ax.set_ylim(-1, total_height + 1)
#     ax.set_aspect('equal')
#     ax.axis('off')

#     ax.set_title("Optimized Floor Plan Layout", fontsize=14, weight='bold')
...
#     st.image(buf, caption="Floor Plan", use_column_width=True)
#     st.download_button("📥 Download Optimized Layout", data=buf, file_name="optimized_floor_plan.png", mime="image/png")
#     plt.close()
...
#         try:
#             with st.spinner("🤖 Generating layout using GPT + GATNet..."):
#                 layout_path = generate_layout_from_prompt(prompt, api_key, carpet_area=carpet_area)

#             if layout_path and os.path.exists(layout_path):
#                 st.success("✅ AI-Based Layout generated!")
#                 st.image(layout_path, caption="🧠 AI Generated Floor Plan", use_column_width=True)

#             with open(layout_path, "rb") as f:
#                 st.download_button("📥 Download AI Layout", data=f, file_name="floor_plan.png", mime="image/png")

#     # Show static comparison layout (always)
#             st.markdown("---")
#             st.info("📐 Static Layout Based on Carpet Area (Reference)")
#             draw_static_layout_with_carpet(carpet_area)
